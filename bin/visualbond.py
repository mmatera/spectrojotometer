...
class FindDialog(Toplevel):
    def __init__(self, app, edt, replace=False):
        ...
        Button(buttonswFrame, text="Find previous", command=self.search_previous).pack(
            side=TOP
        )
        if replace:
            Button(buttonswFrame, text="Replace", command=self.replace).pack(side=TOP)
            Button(buttonswFrame, text="Replace all", command=self.replace_all).pack(
                side=TOP
            )
...
class ImportConfigWindow(Toplevel):
    def __init__(self, app):
        ...
        Button(
            buttons, text="Load Configuration from File", command=self.configs_from_file
        ).pack(side=RIGHT)
        ...
        framebts = Frame(self)
        Button(
            framebts, text="Send to main application", command=self.send_to_application
        ).pack(side=LEFT)
    ...
    def onmodelselect(self, *args):
        if self.selected_model.get() == "[other model]":
            ...
            for key in self.models:
                menu.add_command(
                    label=key, command=lambda value=key: self.selected_model.set(value)
                )
    ...
    def map_confs(self):
        ...
        for line in lines:
            ...
            if len(fields) == 1:
                ...
                continue
            fields = [
                str(float(fields[0]) * self.scale_energy),
...
class ApplicationGUI:
    def __init__(self):
        ...
        self.statusbar = Label(
            self.root, text="No model loaded.", bd=1, relief=SUNKEN, anchor=W
        )
    ...
    def buildmenus(self):
        ...
        editmenu.add_command(
            label="Replace", command=self.call_replace, accelerator="<Control+r>"
        )
        editmenu.add_separator()
        editmenu.add_command(
            label="Clean log",
    ...
    def build_page3(self):
        ...
        row.pack(side=TOP, fill=X)
        btn = Button(
            controls1, text="Estimate Parameters", command=self.evaluate_couplings
        )
        btn.pack()
        controls1.pack(side=TOP, fill=X)
        controls.pack(side=LEFT, fill=Y)
    ...
    def build_page4(self):
        ...
        self.outputformat.set("Plain")
        ctrl = LabelFrame(
            self.page4, text="Evaluate and Show parameters", padx=5, pady=5
        )
        ...
        leftpanel = LabelFrame(
            self.page4, relief=SUNKEN, text="Configurations", padx=5, pady=5
        )
        ScrolledText(leftpanel).pack(side=LEFT, fill=Y)
        leftpanel.pack(side=LEFT, fill=Y)
        rightpanel = LabelFrame(
            self.page4, relief=SUNKEN, text="Results", padx=5, pady=5
        )
        respanel = LabelFrame(
            rightpanel, text="Determined Parameters", relief=SUNKEN, padx=5, pady=5
        )
    ...
    def print_full_equations(self, ev=None):
        ...
        equations = self.model.formatted_equations(
            cm, ensname=None, comments=labels, format=eqformat
        )
        equations = (
            equations
            + "\n\n |"
    ...
    def add_bonds(self):
        if self.model is None:
            messagebox.showerror(
                "Error", "Model was not loaded. " + "Please load a model first.\n"
            )
            return
