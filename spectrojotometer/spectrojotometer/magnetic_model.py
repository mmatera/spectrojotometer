...
class MagneticModel:
    ...
    def compute_couplings(self, confs, energs, err_energs=.01, printeqs=False):
        ...
        model_chi = (coeffs.dot(js) - energs)
        print("model_chi=", model_chi)
        rr = (len(model_chi) - sum(model_chi**2) / err_energs**2)
        if rr < 0:
...
_chemical_name_common                  """ + self.model_label + "\n"
            ...
            if len(bravais_vectors) == 1:
                ...
                gamma = round(180. / 3.1415926 * bravais_vectors[0].dot(bravais_vectors[1]) /
                              (a * b))
                ...
                gamma = round(180 / 3.1415926 * np.arccos(bravais_vectors[0].dot(bravais_vectors[1]) /
                                                          (a * b)))
                alpha = round(180 / 3.1415926 * np.arccos(bravais_vectors[0].dot(bravais_vectors[2]) /
                                                          (a * c)))
                beta = round(180 / 3.1415926 * np.arccos(bravais_vectors[0].dot(bravais_vectors[2]) /
                                                         (c * b)))
                fileout.write("_cell_length_a \t\t\t" + str(a) + "\n")
                fileout.write("_cell_length_b \t\t\t" + str(b) + "\n")
