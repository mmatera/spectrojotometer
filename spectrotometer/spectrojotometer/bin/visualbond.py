...
class ImportConfigWindow(Toplevel):
    ...
    def onmodelselect(self, *args):
        if self.selected_model.get() == "[other model]":
            ...
            # self.optmodels.update_idletasks()
            # self.inputconfs.focus_set()
            # self.optmodels.focus_set()
            self.inputconfs.focus_set()
...
#         self.build_page4()
    ...
    def buildmenus(self):
        self.root.bind_all("<Control-m>", self.import_model)
        ...
        self.filemenu.add_command(label="Open model file...",
                                  command=self.import_model,
                                  accelerator="<Control+m>")
        self.filemenu.add_command(label="Open config file...",
                             command=self.import_configs, state=DISABLED)
        self.filemenu.add_separator()
        self.filemenu.add_command(label="Save model as ...",
                             command=self.save_model, state=DISABLED)
        self.filemenu.add_command(label="Save configurations as ...",
                             command=self.save_configs, state=DISABLED)
        self.filemenu.add_separator()
...
#             ents[field] = ent
    ...
    def import_model(self):
        ...
        self.filemenu.entryconfigure(4, state=NORMAL)
        self.nb.select(0)
    ...
    def import_configs(self, clean=True):
        ...
        self.filemenu.entryconfigure(5, state=NORMAL)
        self.nb.select(1)
...
#             self.modelcif.insert(INSERT, modeltxt)
    ...
    def plot_delta_energies(self, *args):
        ...
        plt.plot(indices, [energy_tolerance for i in indices], linestyle=":")
        plt.plot(indices, [-energy_tolerance for i in indices], linestyle=":")
